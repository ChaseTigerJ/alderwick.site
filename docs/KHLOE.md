...
The original model’s anatomy, attached ears, broad muzzle, faceted coat, skeleton and twelve animations are preserved. The added eye pieces, collar and tag have been removed. Two broad, tapered charcoal brow markings follow the forehead surface and its skin weights. Shared shape keys lift the markings and underlying fur together for visible expressions: an asymmetric curious lift, a questioning seated pose and playful double-brow lifts. There are no eye pieces, collar or tag. These shapes share the skeletal animation clips, crossfades and pause behavior. The island character is 40% smaller than before (root scale 0.516 instead of 0.86), with pawprint size, stride spacing and walking speed reduced to match. Her full-size 404 portrait is unchanged in framing.

`build_khloe.py` imports the vendored model, adds the fitted brow markings and deformations, and saves `khloe.blend`. The editable file retains all twelve original `DreamNoms_*` actions. Five runtime clips are sampled from those original performances:
