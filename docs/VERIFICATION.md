...
## Quiet breeze, clear rigging, and game bookmark artwork

- All 66 focused tests pass, including 5 new tests using the final exported model. They check all 17 trees and both masts across ten minutes of motion, stationary roots/deck pivots, pinned cable endpoints, cloth hoists, and rope clearance across 120 wind samples. Tree sway stays below 0.24 degrees and mast sway below 0.09 degrees.
- The updated Blender source and GLB keep square-sail rigging aft of the fabric and yardarms, with mast collar/cleat ties and deck anchors following the hull taper. Segmented cables flex at their upper ties while their deck/bowsprit ends stay fixed. The final export has 139 meshes, 19,923 triangles, and 1,638,420 bytes. The fallback poster was regenerated.
- Browser inspection covered the enlarged ship from multiple angles, the desktop world controls, and a 390×844 mobile layout. No play/pause controls remain, and no horizontal page overflow or mobile ZIP links were found. The canvas computed touch-action is now none; a legacy pan-y!important rule that could override one-finger orbit has been removed.
- The favicon and Apple/192/512-pixel icons match the game package bytes exactly. PNG dimensions, opaque alpha, HTML links, and relative manifest references were verified; their provenance is recorded in ICONS.md. Physical iOS Home Screen caching and hardware touch gestures were not tested.
- TypeScript and the Vite production build pass. Production browser rendering has no console warnings or errors. The existing dependency directive notices and approximately 697 kB lazy scene chunk warning remain informational. Announcement, desktop download, iOS, and Android switches remain false.


## Sail clearance, visible breeze, synchronized shadows, and water reflections

- All 78 focused Node tests pass. New triangle-level model checks reproduce the previous jib/square-sail and upper/lower-square-sail intersections, and verify clearance throughout the stronger breeze. The final Blender export has 139 meshes, 20,227 triangles, and 1,648,304 bytes; the editable scene and fallback poster were regenerated.
- Trees now stay below 0.9 degrees and masts below 0.4 degrees, with related 8–14 second cycles. A camera-projection test confirms visible tip movement within four seconds at a normal desktop composition. Fixed roots, deck anchors, ropes, and flag attachments remain covered.
- The previous roughly eight-per-second shadow refresh is removed. Matrices update after all animation, then each rendered frame requests one fresh shadow map shared by reflection and main view. Tests cover consecutive ship/mast poses, capture order, and a single outdoor lighting direction throughout day/night transitions.
- Real harbor geometry appears in a softly blurred, lightly rippled water reflection. Capture respects the waterline, alpha page background, camera view offset, effects switch, frozen motion clock, and DPR. Its longest target dimension is capped at 640 pixels on desktop and 384 on coarse-pointer devices. Six focused tests cover alignment, clipping, render state restoration, one shared shadow update, resource disposal, size bounds, and non-unit-DPR viewport behavior.
- Production browser inspection covered both sides of the enlarged sails, ship discovery playback, day/night reflections, and the winter snowglobe. A 390×844 mobile viewport has no horizontal page overflow or ZIP links. No browser console warnings or errors were found. Physical mobile GPU performance and hardware gestures were not tested.
- TypeScript and the Vite production build pass. A model content hash in its request URL prevents stale Blender exports being paired with updated runtime code. Existing dependency directive notices and the approximately 703 kB lazy scene chunk warning remain informational. Announcement and desktop/iOS/Android availability remain false.

## Khloé’s custom 404 page

- All 83 Node tests and the TypeScript/Vite production build pass. Five new tests execute the actual early inline redirect script with a controlled browser clock: exactly five seconds, late DOM/resources, countdown wording, timer cleanup, BFCache restoration, and a fixed homepage destination even with hostile query strings.
- Vite emits a standalone publishing-root `404.html`, isolated CSS, and a 38,222-byte transparent portrait rendered from the existing Khloé model. The error page loads no React or Three.js runtime. Its root base resolves assets from nested missing URLs. A manual home link and JavaScript-disabled meta refresh are included.
- A local static server reproduced HTTP 404 responses instead of the development server’s SPA fallback. Desktop, tablet, and phone layouts were inspected at 1440, 1024, 768, 390, and 320 pixels. Portrait loading, countdown, automatic homepage return, and keyboard activation of the home link were checked; there was no horizontal page overflow.
- Existing island assets, game package, and administrative availability settings are unchanged.

## Khloé’s continuous German Shepherd character

- All 95 Node tests pass, including the final exported skin, normalized weights, bone hierarchy, five clip bindings and durations, sampled pose bounds, animated picking, interrupted crossfades, pause/reduced-motion behavior, one-shot playback, and five-second snowprint expiry. TypeScript and the Vite production build pass.
- Khloé now uses a continuous anatomical surface adapted from a documented CC0 German Shepherd source, with attached ears, a lean torso, fitted facial details, a rose collar, and five authored skeletal clips. The final face pass broadens/deepens the muzzle without extending it, enlarges the fitted eyes, and removes the thin lower-jaw flap. Front, side, three-quarter, seated and moving poses were inspected in Blender and WebGL.
- The canonical character source contains 10 meshes and 5,041 vertices. The complete shipping island GLB contains 124 meshes, 28,633 triangles, one 50-joint skin, all five character clips, and 2,854,288 bytes. The fallback poster and editable island scene were regenerated.
- All 114 non-character meshes match the preceding export exactly after comparing transformed positions, normals, oriented triangles, matrices and material parameters independently of storage order. The ship, buildings, vegetation and other scenery retain their existing geometry.
- Browser checks covered the final exported character's standing, walking, seated/head-tilt and playful poses, the island discovery response, and the 404 page at 1440×900 and 390×844. The updated portrait loaded at full resolution without horizontal page overflow. The local static server returned an actual HTTP 404 for a missing path, and the page returned to the homepage automatically. No application console warnings or errors were found.
- The 404 portrait is 1000×1100 transparent RGBA, 44,362 bytes, and is generated from the same canonical character and seated clip. Model and portrait URLs use content hashes to invalidate old cached artwork. The 404 remains independent of the Three.js runtime and retains its five-second redirect.
- Announcement and desktop/iOS/Android availability flags remain false. The portable game package and domain settings are unchanged. Existing dependency directive notices and the approximately 717 kB lazy scene-chunk warning remain informational. Physical mobile hardware/GPU performance was not measured in these browser viewport checks.
