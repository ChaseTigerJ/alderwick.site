...
The generated desktop reference establishes a cream parchment field, evergreen oversized serif headline on the left, dominant autumn harbor diorama across the right two thirds, orange tactile download CTA, and fine unboxed navigation. Cormorant Garamond suits the game's historical setting; DM Sans keeps controls readable. The 3D scene is the main visual, never a raster substitute for interaction. The title remains actual HTML.
...
Motion communicates the living world and user feedback: water, chimney smoke, scene rotation, changing light, and package opening. Manual drag and explicit day/night, season and pause controls are keyboard-accessible. Reduced motion disables autonomous movement. On mobile the island stacks under the title; download remains in view.

All Blender geometry is original and reproducible from assets-source. Download status is driven by release metadata, never fabricated platform availability.
