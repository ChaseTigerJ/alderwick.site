...
export default function App() {
  ...
  const discover = (id: number) => { if (!worldSettings.discoveriesEnabled || !discoveries[id]) return; setFound(current => current.includes(id) ? current : [...current, id]); setDiscovery(id); setAction({ id, nonce: ++nextAction.current }); };
  ...
  return <>
    ...
    <main id="main">
      ...
      {worldSettings.discoveriesEnabled && <section className="discovery-section" id="discover" aria-labelledby="discovery-title">
        ...
        <div className="discovery-copy"><p>Somewhere in this little harbor, {discoveries.length} small surprises are waiting. Wander, follow your curiosity, and see what answers back.</p>{interfaceSettings.showDiscoveryProgress && <div className="discovery-progress" aria-label={`${found.length} of ${discoveries.length} discoveries found`}>{discoveries.map((_, i) => <span key={i} className={found.includes(i) ? 'complete' : ''}>{found.includes(i) ? <Check size={17} /> : <Sparkle size={17} />}</span>)}<strong>{found.length} / {discoveries.length} found</strong></div>}<a className="text-link" href="#island">{found.length === discoveries.length ? 'Visit your discoveries again' : 'Let’s have a look around'}<ArrowRight size={18} /></a>{interfaceSettings.showDiscoveryProgress && found.length === discoveries.length && <p className="completion-note">A curious soul. You’ll fit right in here.</p>}</div>
      </section>}
